    ...
    <div className="relative min-h-screen overflow-hidden bg-[var(--bg-app)] text-[var(--text-primary)]">
      ...
      <div className="section-shell relative flex min-h-screen flex-col py-8">
        <div className="flex items-center justify-between gap-4">
          <Link to="/" className="inline-flex items-center gap-3 rounded-full border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-2 text-sm font-medium text-[var(--text-secondary)] backdrop-blur-xl hover:text-[var(--text-primary)]">
            <ArrowLeft className="h-4 w-4" />
            Back to home
          </Link>
          <Link to="/" className="flex items-center gap-3">
            <img src={logoMark} alt="ScholarOS" className="h-10 w-10 rounded-2xl shadow-lg shadow-blue-500/20" />
            <span className="text-sm font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">ScholarOS</span>
          </Link>
        </div>
        ...
        <div className="grid flex-1 items-center gap-10 py-10 lg:grid-cols-[1.08fr_0.92fr]">
          <motion.div
            ...
            className="relative hidden rounded-[36px] border border-[var(--border)] bg-slate-950 px-10 py-10 text-white shadow-2xl lg:block"
          >
          ...
          <motion.div
            ...
            className="glass-panel gradient-stroke-premium mx-auto w-full max-w-xl rounded-[36px] px-6 py-8 shadow-xl sm:px-8 sm:py-10"
          >
            <div className="space-y-3">
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--badge-blue)] px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.22em] text-[var(--badge-blue-text)]">
                Welcome to ScholarOS
              </div>
              <h2 className="text-4xl font-semibold tracking-[-0.04em] text-[var(--text-primary)]">{title}</h2>
              <p className="text-base leading-8 text-[var(--text-secondary)]">{subtitle}</p>
            </div>
            <div className="mt-8">{children}</div>
            <div className="mt-8 border-t border-[var(--border)] pt-6 text-sm text-[var(--text-muted)]">
